...
public class AdvertiseService {
    ...
    public void add(Advertise advertise) {
        List<Advertise> advertiseList = advertiseMapper.selectByLocation(advertise.getLocation());
        if (CollectionUtil.isNotEmpty(advertiseList)) {
    ...
    public void updateById(Advertise advertise) {
        List<Advertise> advertiseList = advertiseMapper.selectByLocation(advertise.getLocation());
        if (CollectionUtil.isNotEmpty(advertiseList)) {
            if (!advertise.getId().equals(advertiseList.get(0).getId())) {
                throw new CustomException("-1", "您选择的广告位已经被占用了，请选择其他广告位");
            }
        }
